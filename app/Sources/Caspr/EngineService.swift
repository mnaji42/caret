...
enum EngineService {
    ...
    nonisolated static let label = "fr.lyriastudio.caspr.engine"
    ...
    /// indépendamment — désinstaller Caspr en gardant le modèle laisse
    ...
    nonisolated static var logFile: URL {
        FileManager.default.homeDirectoryForCurrentUser
            .appending(path: "Library/Logs/Caspr/engine.log")
    }
    ...
    /// dans `CasprCore`, où elle est testée : se tromper ici ne produit aucune
    ...
    nonisolated static var unreachableAdvice: String {
        ...
        case .ready:
            ...
            return "le modèle est en cours de chargement en mémoire — jusqu'à "
                ...
                + "si ça persiste, voir ~/Library/Logs/Caspr/engine.log"
        case .serviceMissing:
    ...
    static func reconcile(needed: Bool) {
        ...
        case (true, false):
            NSLog("caspr: démarrage du moteur CrisperWhisper")
            _ = run(["bootstrap", domain, agentPath]) ?? run(["kickstart", "\(domain)/\(label)"])
        case (false, true):
            NSLog("caspr: arrêt du moteur CrisperWhisper — inutilisé")
