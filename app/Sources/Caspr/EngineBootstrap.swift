...
/// dépôt à cloner, plus de `~/.caspr`, plus rien à récupérer sur GitHub pour
...
///
/// Rien n'est installé hors du dossier de Caspr. Ni `/usr/local`, ni
...
final class EngineBootstrap {
    ...
    /// Tout vit sous le dossier de Caspr, jamais dans le bundle.
    ///
    /// L'environnement Python ne peut pas habiter `Caspr.app` : la mise à
    ...
    static var supportDirectory: URL {
        FileManager.default.homeDirectoryForCurrentUser
            .appending(path: "Library/Application Support/Caspr")
    }
    ...
    static var obstacle: String? {
        ...
        guard bundledSource != nil else {
            return "Cette copie de Caspr ne contient pas le code du moteur. "
    ...
    /// absolu. Celui de Caspr passe en premier, les deux de Homebrew ensuite,
    ...
    /// Recopie le moteur du bundle vers le dossier de Caspr.
    ///
    /// Refaite à chaque installation, pour qu'une mise à jour de Caspr porte
                ...
                class Progress(_tqdm):
                    ...
                    def update(self, n=1):
                        ...
                        if self._bytes and n:
                            Progress.done += n
                            print("CASPR_PROGRESS %d" % Progress.done,
    ...
    private static let progressMarker = "CASPR_PROGRESS "
    ...
    /// touche jamais à celui du système. C'est aussi ce que Caspr voulait
    ...
    private static func prepareShims() throws {
        ...
        let script = """
        #!/bin/sh
        # Posé par Caspr — cf. EngineBootstrap.prepareShims.
    ...
    private enum Failure: LocalizedError {
        ...
        var errorDescription: String? {
            switch self {
            case .missingSource:
                "Le code du moteur est absent de cette copie de Caspr."
            case .releaseUnreadable:
            ...
            case .slowStart:
                "Le service a démarré mais n'a pas fini de charger le modèle "
                    ...
                    + "~/Library/Logs/Caspr/engine.log pour savoir sur quoi "
                    + "il bute."
            }
        }
    }
...
extension EngineBootstrap {
    ...
    private nonisolated static func latestToolRelease() async throws -> ToolRelease {
        ...
        request.setValue("Caspr", forHTTPHeaderField: "User-Agent")
...
private final class LineCollector {
    ...
    private let queue = DispatchQueue(label: "fr.lyriastudio.caspr.lines")
