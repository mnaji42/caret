...
#
#     identifier "dev.mnaji.caret" and certificate leaf = H"<empreinte>"
#
...
openssl pkcs12 -export \
    ...
    -passout pass:caret -name "$CERT_NAME" 2>/dev/null

# -T /usr/bin/codesign autorise codesign à utiliser la clé sans redemander
# l'accès au trousseau à chaque signature.
security import "$TMP/cert.p12" -k "$KEYCHAIN" -T /usr/bin/codesign -P caret >/dev/null
